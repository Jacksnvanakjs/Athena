...
def _build_prompt(items: list[RawItem]) -> str:
    ...
    return (
        ...
        "- 媒体点名：上市云算力商（Rum Group/RUM/Rumble/Northern Data 等）大额 GPU/compute "
        "租赁客户是 Anthropic/OpenAI → relevant=true；beneficiary=RUM，anchor=Anthropic；"
        "即便标题写 shares gain/盘前大涨，正文有金额+算力协议也判 true，llm_score≥80\n"
        "- Anthropic/OpenAI × 上市算力云（CoreWeave/CRWV、Nebius/NBIS、IREN、Applied Digital）"
        "多年 GPU 容量/租赁 → relevant=true，受益方取上市云算力商\n"
        "【模式负例】\n"
        ...
        "- 分析师「地位/谁在赢」且无新签 → false：Marvell Is Winning Custom AI…、"
        "TSM occupies powerful position… currently the sole wafer supplier、"
        "Record Month Confirms… 等复述既有代工/客户关系、无 signed/announced agreement → false\n"
        ...
        "角色与打分：\n"
        "- anchor=叙事核心/更大一方（云厂、大模型、Nvidia、代工厂 TSMC 等）；可以是未上市公司。"
        "**禁止** anchor 与 beneficiary 为同一公司。\n"
        "- 例：正文写 Marvell 定制芯片、TSMC 为现有 sole wafer supplier → "
        "若无新签协议则 relevant=false；若确有新容量/新节点签约，"
        "beneficiary=Marvell，anchor=TSMC（或 hyperscaler），勿把 TSMC 填成受益方又填成锚点。\n"
